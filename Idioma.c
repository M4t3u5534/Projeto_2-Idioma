...
int main(){
    //Criação do vetor para quantar a quantidade de letras e para input da frase digitada
    int letras[26] = {0};
    char frase[200] = {' '};
    ...
    for (int i=0;i<26;i++)
    {
        //cálculo da porcentagem da letra na frase digitada
        porc = letras[i]*100/tamanho;
        //incrementação da diferença entre a porcentagem da frase e a do idioma
        porx_port += abs(porc - f_portugues[i]);
        porx_ingl += abs(porc - f_ingles[i]);
    }

    //garante que diferênça de proximidade entre os idiomas não seja muito pequena
    if (abs(porx_port - porx_ingl) < 1)
    {
